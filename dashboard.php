...
if (isset($_GET['delete-id'])) {
    $id = $_GET['delete-id'];
    $q = 'DELETE FROM `cms` . `user` WHERE `srno`="'.$id.'"';
    mysqli_query($con, $q);
}

$sql = "SELECT * FROM `cms` . `user` ORDER BY dt DESC";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        array_push($datetimes, $row['dt']);
        array_push($names, $row['name']);
        array_push($messages, $row['message']);
        array_push($ids, $row['srno']);
    }
}
        ...
        <div class="sidebar">
            <div class="sidebar-header">
                <div class="app-icon" style="display: flex; align-items: center">
                    ...
                    <h4 style="margin-left: 10px; color: white"><a style="color: white" href="service.php">Home</a></h4>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        for ($i=0; $i<count($names); $i++) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $messages[$i] ?></td>
                                <td><?php echo $datetimes[$i] ?></td>
                                <td><a class="btn btn-danger" href="./dashboard.php?delete-id=<?php echo $ids[$i] ?>">Delete</a></td>
                            </tr>
